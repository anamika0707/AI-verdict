...
app.post("/analyze", async (req, res) => {
  try {
    const { caseTitle, text } = req.body;

    if (!text || !text.trim()) return res.status(400).json({ error: "No arguments provided" });
    ...
    if (!output) throw new Error("No text returned from Gemini API");
